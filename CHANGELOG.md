...
### Added
- **Web app shortcuts** – Add tiles for any local-network web app (recipe manager, home automation, media server, etc.) from Settings → 🔗 Apps. Each tile opens in a full-screen iframe overlay with a keyboard toggle. The `.dash-apps` tile strip hides automatically when no apps are configured. Stored in `app/data/settings.json` under `webApps: WebApp[]`.
- **Emoji icon picker for web app shortcuts** – The Apps form shows a grid of 28 tappable emoji buttons instead of a text input; the selected emoji highlights in blue. Works for both adding and editing apps.
- **Inline editing for web app shortcuts** – Each app in Settings → 🔗 Apps has an Edit button that expands the row into a pre-filled form (emoji picker, name, URL) with Save/Cancel. No need to delete and re-add to fix a typo.
- **Inline editing for to-do items** – Each item in Settings → ✅ To-Do now has a ✎ pencil button. Clicking it opens an inline input pre-filled with the current text; Enter or Save commits, Escape or Cancel discards. Backed by a new `editItem()` pure function in `lib/todo.ts`.
...
- **Web app settings overwrite bug** – Opening Settings on a remote browser (e.g. PC accessing the Pi) before server sync completed would save with an empty `webApps` list, erasing shortcuts added on the Pi. The Settings panel now calls `syncSettingsFromServer()` each time it opens and waits for the result before rendering.
- **Lint: 10 pre-existing errors eliminated** – `photos.ts`: replaced 6 `any` casts with proper interfaces (`GisWindow`, `GisTokenClient`, `GisTokenClientConfig`, `GisTokenResponse`) for the Google Identity Services script API; typed request body as `Record<string, unknown>`; typed API response directly. `settings.ts` / `todo.ts`: added explanatory comments to 4 empty `catch {}` blocks to satisfy the `no-empty` ESLint rule. `npm run lint` now exits clean.
